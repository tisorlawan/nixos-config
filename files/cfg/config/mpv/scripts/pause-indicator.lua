	...
	__migrate = function(self)
		...
		if oldConfig and not newConfig then
			...
			if dirExists and not utils.readdir(configDir) then
				log.warn(
					("Configuration migration failed. %q exists and does not appear to be a folder"):format(configDir)
				)
				return
			else
...
do
	...
	_class_0 = setmetatable({
		__init = function(self)
			...
			self.updateTimer = mp.add_periodic_timer(
				settings["redraw-period"],
				(function()
					local _base_1 = self
...
do
	...
	local _base_0 = {
		update = function(self, now)
			if self.isReversed then
				self.linearProgress =
					math.max(0, math.min(1, self.linearProgress + (self.lastUpdate - now) * self.durationR))
				if self.linearProgress == 0 then
					self.isFinished = true
				end
			else
				self.linearProgress =
					math.max(0, math.min(1, self.linearProgress + (now - self.lastUpdate) * self.durationR))
	...
	_class_0 = setmetatable({
		__init = function(self, initialValue, endValue, duration, updateCb, finishedCb, accel)
			...
			self.initialValue, self.endValue, self.duration, self.updateCb, self.finishedCb, self.accel =
				initialValue, endValue, duration, updateCb, finishedCb, accel
